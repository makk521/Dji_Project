...
void consumerFun(ThreadSafeQueue<std::string>& sharedQueue, int uavSocket, sockaddr_in uavAddr) {
    ...
    */
    ...
    // 连接到无人机
    MyStruct dataToSend;
    dataToSend.intValue = 42;
    dataToSend.floatValue = 3.14f;
    std::strcpy(dataToSend.stringValue, "Hello, Server!");
    ...
    while (true) {
        ...
        // int bytesSent = send(uavSocket, value.c_str(), value.length(), 0);
        int bytesSent = send(uavSocket, &dataToSend, sizeof(dataToSend), 0);
        if (bytesSent == -1) {
            perror("Error sending data");
            break;
        }
        cout << "Sent data: " << value << endl;
    }
}
