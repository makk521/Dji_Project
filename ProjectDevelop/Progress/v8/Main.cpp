...
void postData(ThreadSafeQueue<DataPack>& otherModuToInfoAnalyQueue) {
    ...
    */
    ...
    uavAddr.sin_port = htons(8002); // 替换为服务器的端口号
    ...
    while (true) {
        if (!otherModuToInfoAnalyQueue.empty()) {
            ...
            std::cout << "信息解析模块收到集群管理模块等三个模块的返回数据并准备发送" << std::endl;
...
int main() {
    ...
    std::thread ipUpdateTimeThread(IPupdate_Time, std::ref(otherModuToInfoAnalyQueue));
