// 天翼云向腾讯云发送，接收使用5000端口，发送目标为124.223.76.58:5000，
// 运行顺序为先运行，为保证post成功，post函数先停10秒再运行，即腾讯云方面需要10秒之内运行
#include <iostream>
#include <cstring>
#include <unistd.h>
...
void postData(int clientSocket, sockaddr_in serverAddr){
    usleep(10000000);
    std::cout << "Post delay 10 seconds" << std::endl;
    // 连接到服务器
...
int main() {
    ...
    serverAddrPoster.sin_addr.s_addr = inet_addr("124.223.76.58"); // 替换为服务器的 IP 地址
