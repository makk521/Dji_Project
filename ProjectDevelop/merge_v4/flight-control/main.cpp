    ...
    */
    ...
    while (true) {
        ...
        else if (!commProcToInfoAnalyQueue.empty())
        {
            ...
            std::cout << "取出commProcToInfoAnalyQueue" << value.payload << std::endl;
            std::cout << "准备发送" << std::endl;
            char* dataToSend = static_cast<char*>(malloc(2000));
            ...
            // int bytesSent = send(uavSocket, &dataToSendPacK, sizeof(dataToSendPacK), 0);
            std::cout << "执行指令情况等待发送" << std::endl;           
            int bytesSent = send(uavSocket, dataToSend, 2000, 0);  // int socket, const void *buffer, size_t length, int flags
            std::cout << "执行指令情况等已发送" << std::endl;
            if (bytesSent == -1) {
                perror("Error sending data");
                break;
            }
            // std::cout << "已发送数据：" << value.payload << std::endl;
...
int main() {
    ...
    std::thread orderThread(order, std::ref(commProcPriorityQueue));
    ...
    */
    while (true) {
        if (!commProcToInfoAnalyQueue.empty()) {
            ...
            std::cout << "信息解析模块收到指令处理模块的返回数据:"<< ptr.payload << std::endl;
        }
    }
}
...
void commProcListenSubData(){
    /**
    * @brief 指令处理模块监听订阅信息队列subDataToCommProcQueue并将其放入队列commProcToInfoAnalyQueue
    * @param None
    * @return None
    */
    while (true) {
        if (!subDataToCommProcQueue.empty()) {
            ...
            std::cout << "指令处理模块收到订阅信息并将其传给信息解析模块,数据为" << ptr.payload << std::endl;
        }   
    }
}
