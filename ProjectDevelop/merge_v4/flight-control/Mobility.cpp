...
/*
//一次性读取多台无人机的IP信息
vector<string> Read_UAVinfo_IP_mul(vector<int> &uavID){
    int n = uavID.size();
    vector<string> uavIP(n);
    string ReadJsonString;
    json ReadUavJson;
    for(int i = 0 ; i<n ; i++){
        ReadJsonString = Read_UAVinfo(uavID[i]);
        ReadUavJson = nlohmann::json::parse(ReadJsonString);//反序列化
        uavIP[i] = ReadUavJson["uav_ip"];
        //cout << "[Mobility]ID为" << uavID <<"的无人机对应的IP地址为：" << ReadUavJson["uav_ip"] <<endl;
    }
	return uavIP;
}
