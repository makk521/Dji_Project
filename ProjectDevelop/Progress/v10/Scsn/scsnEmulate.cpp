...
void multiProgress(ThreadSafeQueue<std::string>& sharedQueue, ThreadSafeQueue<std::string>& sendDataQueue){
    /**
    * @brief 马际清监听sharedQueue并处理编队信息，将单个无人机的指令(字符串)放入sendDataQueue
    * @param sharedQueue 与函数receivePythonData共享
    * @param sendDataQueue 与函数consumerFun共享
    * @return None
    */
    while (true) {
        if (!sharedQueue.empty()) {
            std::string value = sharedQueue.pop(); // 取出未处理的字符串{'uid': '2', 'uavType': '', 'action': 'takeoff', 'params': [], 'commandNum': ''}
            std::cout << "取出未处理的字符串" << value << std::endl;
        }
    }
}
